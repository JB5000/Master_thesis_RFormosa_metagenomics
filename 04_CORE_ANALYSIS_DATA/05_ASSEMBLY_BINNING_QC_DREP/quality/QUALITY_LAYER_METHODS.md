...
CheckM2 ran inside the recovered nf-core/mag v5.4.1 workflows (`--run_checkm2`), with legacy CheckM disabled (`--run_checkm false`). A primary CheckM2 process log from a final run reports **CheckM2 v1.1.0**, a supplied `uniref100.KO.1.dmnd` database, and successful completion. The database path is recorded; its release/hash remains **UNVERIFIABLE**.
...
GUNC was enabled in nf-core/mag and primary GUNC summaries exist. The 56-input manifest is defined by CheckM2 completeness/contamination. Among these 56 inputs, 42 have `pass.GUNC=True`; 14 have no recovered GUNC result. No GUNC exclusion field is present in the retained 56-input manifest. GUNC version and complete final coverage have status **UNVERIFIABLE**.
